...
Hertie_frame <- data_frame(Address = c("Hertie School of Governance, Friedrichstrasse, Berlin, Germany"))


Hertie_frame['Geocode_x']<- geocode_OSM(address_hertie)$coords["x"]
Hertie_frame['Geocode_y']<- geocode_OSM(address_hertie)$coords["y"]

hertie_as_a_point <- st_as_sf(Hertie_frame, coords = c(x = 'Geocode_x', 'Geocode_y'),
                              crs = 4326)
...
tm_shape(filter(World, iso_a3 == "DEU"))+
  tm_fill() +
  tm_borders() +
tm_shape(hertie_as_a_point) +
  tm_bubbles(col = "red", size = .025, labels = c('Hertie'))

## This map is pretty boring and ad-hoc. Let's work with something more advanced. 
